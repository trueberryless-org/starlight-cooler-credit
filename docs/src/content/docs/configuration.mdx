...
The `customImage` option allows you to provide a custom image URL for the Cooler Credit component. If provided, the default image will be replaced with the custom image. The configuration option [`showImage`](#showimage) must be set to `true` for the image to be displayed.
