...
This option accepts a path to a local image relative to the root of your project. Relative paths must start with `./`:

```ts
starlightCoolerCredit({
  customImage: "./src/assets/rainsberger.ca.webp",
  customImageAlt: "The rainsberger.ca logo",
}),
```
...
:::note
This option is required if [`customImage`](#customimage) is specified.
:::
